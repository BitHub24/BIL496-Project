...
class Node:
    def __init__(self, id: int, lat: float, lon: float):
        ...
        self.lat = lat
        self.lon = lon
        self.adjacent: Dict[int, Dict[str, float]] = {}  # node_id -> {distance, speed_limit}
        self.turn_restrictions: Dict[Tuple[int, int], bool] = {}  # (from_id, to_id) -> allowed

    def add_edge(self, node_id: int, distance: float, speed_limit: float = 50.0):
        """Add edge with distance and speed limit (km/h)."""
...
class RoutingGraph:
    ...
    def add_edge(self, from_id: int, to_id: int, bidirectional: bool = True, speed_limit: float = 50.0):
        """Add edge with speed limit (km/h)."""
        ...
        from_node.add_edge(to_id, distance, speed_limit)
        if bidirectional:
            to_node.add_edge(from_id, distance, speed_limit)
    ...
    def astar(self, start_id: int, goal_id: int, current_time: datetime = None) -> Tuple[List[int], float]:
        """A* path finding algorithm with turn restrictions and traffic."""
        ...
        while open_set:
            ...
            for neighbor_id, edge_data in current_node.adjacent.items():
                ...
                # Calculate time cost based on distance, speed limit and traffic
                distance = edge_data['distance']
                speed = edge_data['speed_limit']
                time_cost = (distance / speed) * traffic_mult  # hours
