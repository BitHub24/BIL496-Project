...
def get_latest_traffic_data(request):
    ...
    try:
        ...
        if not data_files:
            logger.info("No traffic data files found.")
        ...
        logger.error(f"Error decoding JSON from file {latest_file}: {jde}")
        return Response({"error": f"Bozuk trafik veri dosyası bulundu: {os.path.basename(latest_file)}"}, status=500)
    except Exception as e:
